...
// @version      1.0
// @description  Automates unfollowing non-reciprocal accounts with safe random delays (30-210s).
// @author       IG-Studio
...
(function() {
    ...
    async function processCurrentPage() {
        ...
        if (currentPath === currentTarget) {
            console.log("[IG-Studio] Target Match: @" + currentTarget);
            ...
            console.log("[IG-Studio] Success. Sleeping " + delay + "s...");
            await sleep(delay * 1000);
            if (index < TARGETS.length) window.location.href = 'https://www.instagram.com/' + TARGETS[index] + '/';
        } else {
            console.log("[IG-Studio] Redirecting to next target: @" + currentTarget);
    ...
    function injectUI() {
        ...
        panel.style.cssText = "position: fixed; bottom: 30px; right: 30px; z-index: 999999; background: #0f172a; color: #fff; padding: 20px; border-radius: 16px; box-shadow: 0 10px 50px rgba(0,0,0,0.8); font-family: sans-serif; border: 2px solid #e1306c; width: 220px; display: block !important; visibility: visible !important;";
